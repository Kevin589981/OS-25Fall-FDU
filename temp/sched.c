...
void init_sched()
{
    ...
    for (int i=0;i<NCPU;i++){
        ...
        init_spinlock(&temp->lock);
        // 初始化调度器
        temp->run_queue.rb_node=NULL;
        temp->task_count=0;
...
void acquire_sched_lock()
{
    ...
    struct sched *this_cpu_sched=&cpus[cpuid()].sched;
    acquire_spinlock(&this_cpu_sched->lock);
}
...
void release_sched_lock()
{
    ...
    struct sched *this_cpu_sched=&cpus[cpuid()].sched;
    release_spinlock(&this_cpu_sched->lock);
}
...
bool activate_proc(Proc *p)
{
    ...
    // if the proc->state if SLEEPING/UNUSED, set the process state to RUNNABLE and add it to the sched queue
        ...
        p->state=RUNNABLE;
        _rb_insert(&p->schinfo.node, &cpus[cpuid()].sched.run_queue, compare_runtime);
...
static void update_this_state(enum procstate new_state)
{
    ...
    if (new_state==RUNNABLE){
        //!更新runtime
        _rb_insert(&this->schinfo.node,&cpus[cpuid()].sched.run_queue,compare_runtime);
        cpus[cpuid()].sched.task_count++;
...
static Proc *pick_next()
{
    ...
    struct sched *temp=&cpus[cpuid()].sched;
    rb_node next_node=_rb_first(&temp->run_queue);
    if (next_node){
        _rb_erase(next_node, &temp->run_queue);
        temp->task_count--;
...
//     printk("this cpu is %lld, process's pid is %d\n",cpuid(), this->pid);
//     printk("cpu0 is running %d\n",cpus[0].sched.current_proc->pid);
//     // printk("cpu0 is running %d",cpus[0].sched.current_proc->pid);
// #endif
    ...
    if (!next->idle)next->state = RUNNING;
    if (next != this) {
